...
class FormDangKy extends Component {
    ...
    handleChange = (event) => {
        ...
        let { className, id, name, value } = event.target;//id:''taiKhoan,name:'nguyen van a'
        //data-type <input data-type="" /> ==> dù nằm trong thẻ input nhưng ko phải là property ==> 0 dùng event.target ['data-type'];
        ...
        let messError = '';
        if (value === '') {
            messError = id + 'không được bỏ trống phần này!!!'
        };
        if (dataType === 'email') {
    ...
    render() {
        ...
        let { taiKhoan, matKhau, hoTen, soDienThoai, email, loaiNguoiDung } = this.state.value;
        return (
                ...
                <div className='card'>
                    ...
                    <div className='card-body'>
                        <div className='row'>
                            <div className='col-6'>
                                ...
                                <div className='from-group'>
                                    <p>Họ tên</p>
                                    <input className='form-control' id='hoTen' name='hoTen'
                                        onChange={this.handleChange} value={hoTen} />
                                    <p className='text text-danger'>{this.state.error.hoTen}</p>

                                </div>
                                <div className='from-group'>
                                    <p>Email</p>
                                    <input data-type='email' className='form-control' id='email'
                            ...
                            <div className='col-6'>
                                <div className='from-group'>
                                    <p>Mật khẩu</p>
                                    <input className='form-control' id='matKhau' type={'password'}
                                        name='matKhau' onChange={this.handleChange} value={matKhau} />
                                    <p className='text text-danger'>{this.state.error.matKhau}</p>

                                </div>
                                <div className='from-group'>
                                    <p>Số điện thoại</p>
                                    <input data-type='phone' className='form-control' id='soDienThoai'
                                        name='soDienThoai' onChange={this.handleChange} value={soDienThoai} />
                                    <p className='text text-danger'>{this.state.error.soDienThoai}</p>

                                </div>
                                <div className='from-group'>
                                    <p> Loại người dùng</p>
                                    <select className='form-control' id='loaiNguoiDung'
                                        name='loaiNguoiDung' onChange={this.handleChange} value={loaiNguoiDung} >
                    ...
                    <div className='card-footer'>
                        ...
                        <button type='button' className='btn btn-primary ml-2' onClick={()=>{
                            //Gửi dữ liệu sau khi ng dùng cập nhật lên redux
                            const action ={

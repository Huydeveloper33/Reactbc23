...
export default class BaiTapXemChiTiet extends Component {
    ...
    arrPhone = [
        {
            ...
            "cameraTruoc": "20 MP", "cameraSau": "Chính 48 MP & Phụ 8 MP, 5 MP", "ram": "4 GB", "rom": "64 GB", "giaBan": 5700000,
            "hinhAnh": "../img/vsphone.jpg"
        },
        {
            "maSP": 2, "tenSP": "Meizu 16Xs", "manHinh": "AMOLED, FHD+ 2232 x 1080 pixels", "heDieuHanh": "Android 9.0 (Pie); Flyme",
            "cameraTruoc": "20 MP", "cameraSau": "Chính 48 MP & Phụ 8 MP, 5 MP", "ram": "4 GB", "rom": "64 GB", "giaBan": 7600000,
    ...
    state = {
        ...
        gioHang: [
            { maSP: 1, tenSP: "Xmas", hinhAnh: 'https://picsum.photos/100', giaBan: 20000, soLuong: 2 },
    ...
    tangGiamSoLuong = (maSPClick,soLuong) => {//+1 => nút +, -1 => nút -
       ...
       if(spGH){
           ...
           if(spGH.soLuong<1){
               if(window.confirm('Bạn có muonns xóa sản phẩm này không?')){
                   gioHang= gioHang.filter(sp=>sp.maSP !==maSPClick)
               }
               //Nếu ng dùng cancel => --số lượng đó
               spGH.soLuong -= soLuong
           }
       }
    ...
    xoaGioHang = (maSanPhamClick) => {
        ...
        if (index !== -1) {
            //Hỏi ng dùng trc khi thực hiện
            if (window.confirm('Bạn có muốn xóa sản phẩm đang chọn?')) {
                if (window.confirm('Thiệt hông?')) {
    ...
    //state đặt ở đâu thì hàm setState đặt ở đó
    themGioHang = (sanPhamClick) => {//spClick :Chưa có thuộc tính về số lượng 
        ...
        let spGH = gioHang.find(sp => sp.maSP ===
            sanPhamClick.maSP)
        if (spGH) {
            spGH.soLuong += 1;
        } else {
    ...
    render() {
        ...
        let { maSP, tenSP, giaBan, hinhAnh, heDieuHanh,
            manHinh, ram, rom, cameraSau, cameraTruoc } = this.state.spChiTiet

        return (
            <div className='container'>
                <h3 className='mt-5'>Giỏ hảng</h3>
                {/* <button onClick={()=>{
                    this.xoaGioHang()
                }}>Xóa giỏ hàng</button> */}
